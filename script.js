...
try {
    ...
    if (lineLayer && core) {
      const drawLines = () => {
        try {
          ...
          console.error('Error drawing constellation lines:', err);
        }
      };
      let lineTimer;
  ...
  console.error('Constellation lines feature error:', err);
}
...
try {
  ...
  console.error('World map feature error:', err);
}
